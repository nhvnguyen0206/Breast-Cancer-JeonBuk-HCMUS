...
@torch.no_grad()
def evaluate(model, loader, device, amp, return_predictions=False):
    model.eval()
    y_true, y_pred, y_score, case_ids = [], [], [], []
    ...
    for batch in loader:
        ...
        probabilities = torch.softmax(outputs["flat_logits"].float(), dim=1).cpu()
    ...
    if return_predictions:
        return metrics, {
            "case_id": case_ids, "y_true": y_true, "y_pred": y_pred,
            "probabilities": y_score,
        }
    return metrics


def _write_predictions(path: Path, predictions: dict) -> None:
    with path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(["case_id", "y_true", "y_pred", "prob_A", "prob_B", "prob_C", "prob_D"])
        for case_id, truth, prediction, probabilities in zip(
            predictions["case_id"], predictions["y_true"],
            predictions["y_pred"], predictions["probabilities"],
        ):
            writer.writerow([case_id, truth, prediction, *probabilities])
...
def run_training(config: dict, output_dir: Path) -> None:
    ...
    transform_options = config["data"].get("preprocessing")
    ...
    for epoch in range(1, int(config["training"]["epochs"]) + 1):
        ...
        metrics, predictions = evaluate(
            model, valid_loader, device, amp, return_predictions=True
        )
        scheduler.step()
        ...
        if improved:
            ...
            _write_predictions(output_dir / "best_predictions.csv", predictions)
        else:
            patience_counter += 1
        ...
        epoch_record = {
            ...
            "valid_qwk": round(metrics["qwk"], 4),
            "valid_auc_macro": round(metrics["auc_macro_ovr"], 4),
