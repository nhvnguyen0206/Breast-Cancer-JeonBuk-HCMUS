...
def compute_metrics(y_true, y_pred, y_score=None) -> dict:
    ...
    if y_score is not None:
        ...
        one_hot = np.eye(NUM_CLASSES, dtype=np.int64)[truth]
        per_class_auc, curves = {}, {}
        for i in labels:
            try:
                per_class_auc[INDEX_TO_LABEL[i]] = float(
                    roc_auc_score(one_hot[:, i], scores[:, i])
                )
                fpr, tpr, thresholds = roc_curve(one_hot[:, i], scores[:, i])
                curves[INDEX_TO_LABEL[i]] = {
                    "fpr": fpr.tolist(),
                    "tpr": tpr.tolist(),
                    "thresholds": thresholds.tolist(),
                }
            except ValueError:
                per_class_auc[INDEX_TO_LABEL[i]] = None
                curves[INDEX_TO_LABEL[i]] = {"fpr": [], "tpr": [], "thresholds": []}
        result["auc_macro_ovr"] = float(
            roc_auc_score(one_hot, scores, average="macro", multi_class="ovr")
        )
        result["auc_weighted_ovr"] = float(
            roc_auc_score(one_hot, scores, average="weighted", multi_class="ovr")
        )
        result["auc_per_class"] = per_class_auc
