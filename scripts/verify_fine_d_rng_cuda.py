#!/usr/bin/env python3
"""Prove A49 preserves A42's stochastic stream on an allocated CUDA device."""
...
def build(*, fine_d_expert=False, isolate_fine_d_rng=False):
    return DensityModel(
        pretrained=False,
...
def main():
    ...
    output_exact = {
        key: torch.equal(expected[key], actual[key]) for key in expected
    }
    checks = {
    ...
    if not all(checks.values()):
        raise RuntimeError(f"CUDA RNG parity failed: {checks}; {output_exact}")

    treatment.zero_grad(set_to_none=True)
    torch.manual_seed(321)
    outputs = treatment(views)
    loss, _ = MultiTaskLoss([2, 3, 4, 5]).to(device)(
        outputs, torch.tensor([2, 3], device=device)
    )
    loss.backward()
    gradient = treatment.d_fine_head.weight.grad
    gradient_sum = float(gradient.abs().sum())
    if not torch.isfinite(gradient).all() or gradient_sum <= 0:
        raise RuntimeError("Fine-D head did not receive a finite non-zero gradient")
    ...
    print(json.dumps({
        ...
        **checks,
        ...
        "fine_d_head_gradient_l1": gradient_sum,
    }, indent=2))
