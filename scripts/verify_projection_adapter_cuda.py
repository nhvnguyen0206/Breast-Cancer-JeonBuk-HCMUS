#!/usr/bin/env python3
"""Verify A50 starts bit-exact to A42 and learns on an RTX 5090."""
...
def build(*, projection_adapters=False):
    return DensityModel(
        pretrained=False,
...
def main():
    ...
    torch.manual_seed(42)
    treatment = build(projection_adapters=True).to(device).train()
    ...
    if not all(checks.values()):
        raise RuntimeError(f"A50 parity failed: {checks}; {output_exact}")
    ...
    print(json.dumps({
        ...
        **checks,
        ...
        "adapter_output_gradient_l1": gradient_sum,
