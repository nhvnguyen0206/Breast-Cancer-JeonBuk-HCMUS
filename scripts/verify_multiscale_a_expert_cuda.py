#!/usr/bin/env python3
"""Verify A52 starts bit-exact to A42 and learns on an RTX 5090."""
...
def build(*, multiscale_a_expert=False):
    return DensityModel(
        ...
        spatial_grid_size=4, multiscale_a_expert=multiscale_a_expert,
    )
...
def main():
    ...
    torch.manual_seed(42)
    treatment = build(multiscale_a_expert=True).to(device).train()
    ...
    if not all(checks.values()):
        raise RuntimeError(f"A52 parity failed: {checks}; {output_exact}")
