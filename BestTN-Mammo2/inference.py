"""Inference script for BestTN-Mammo2 (E3_E7_CLEAN_RETEST_131).

Reference Performance: Clean Test N=131, Macro-F1 = 0.7438, QWK = 0.7862, Accuracy = 0.7328
"""
...
def main():
    parser = argparse.ArgumentParser(description="BestTN-Mammo2 Inference")
    ...
    parser.add_argument("--exclude-case", type=str, default=None, help="Exclude specific corrupted case ID")
    ...
    exclude_case = args.exclude_case or cfg["ensemble"].get("exclude_case")
    ...
    # Clean filtering if specified
    if exclude_case:
        filtered_indices = [idx for idx, row in ds_512.dataframe.iterrows() if str(row.get("case_id", "")) != str(exclude_case)]
        print(f"[INFO] Filtered out case {exclude_case}: {len(ds_512.dataframe)} -> {len(filtered_indices)} cases")
        ds_512.dataframe = ds_512.dataframe.iloc[filtered_indices].reset_index(drop=True)
        ds_1024.dataframe = ds_1024.dataframe.iloc[filtered_indices].reset_index(drop=True)
    ...
    if y_true is not None:
        ...
        print("\n==================================================")
        print("BestTN-Mammo2 INFERENCE RESULTS (CLEAN)")
        print("==================================================")
        print(f"Cohort Samples    : {len(y_true)}")
