...
class MultiTaskLossOptions:
    ...
    lambda_bc: float = 0.0
    bc_class_counts: list[int] | None = None
    lambda_bc_main: float = 0.0
...
class MultiTaskCriterion(nn.Module):
    def __init__(
        ...
        *,
        ...
        # BC auxiliary loss (B-vs-C)
        if (
            options.lambda_bc > 0
            and options.bc_class_counts is not None
        ):
            self.bc_loss: ClassBalancedFocalLoss | None = (
                ClassBalancedFocalLoss(
                    options.bc_class_counts,
                    beta=beta,
                    gamma=gamma,
                )
            )
    ...
    def forward(
        ...
        if self.options.lambda_bc > 0:
            bc_logits = outputs.get("bc_logits")
            
            if bc_logits is None:
                raise ValueError("BC loss enabled but bc_logits are missing.")
                
            # Only compute on B(1) and C(2) samples
            mask = (labels == 1) | (labels == 2)
            
            if bool(mask.any()):
                bc_targets = labels[mask] - 1 # B -> 0, C -> 1
                bc = self.bc_loss(
                    bc_logits[mask],
                    bc_targets,
                )
            else:
                bc = bc_logits.sum() * 0.0
                
            total = total + self.options.lambda_bc * bc
            parts["bc"] = bc.detach()
            
        if getattr(self.options, "lambda_bc_main", 0.0) > 0:
            mask = (labels == 1) | (labels == 2)
            if bool(mask.any()):
                bc_main_logits = flat_logits[mask][:, [1, 2]]
                bc_targets = labels[mask] - 1
                bc_main = F.cross_entropy(bc_main_logits, bc_targets)
            else:
                bc_main = flat_logits.sum() * 0.0
                
            total = total + self.options.lambda_bc_main * bc_main
            parts["bc_main"] = bc_main.detach()
