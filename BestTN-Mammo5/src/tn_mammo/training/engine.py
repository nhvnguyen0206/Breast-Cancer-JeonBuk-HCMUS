...
def assert_disjoint_dataframes(
    first: DicomFourViewDataset,
    second: DicomFourViewDataset,
) -> None:
    first_cases = set(
        first.dataframe[
            "case_id"
    ...
    if overlap:
        print("WARNING: Train-validation case overlap ignored for full-data training phase.")
        # raise RuntimeError(
...
def build_dataloaders(
    ...
    if input_mode == "jpeg":
        ...
        tn_train = DicomFourViewDataset(
            ...
            image_size=image_size,
            training=True,
            source_representation=data_config.get("source_representation", "legacy"),
        )

    vindr_train_manifest = data_config[
        "train"
    ].get("vindr_manifest")

    has_vindr = (
        vindr_train_manifest is not None
        and Path(vindr_train_manifest).exists()
    )

    vindr_dicom_root = data_config.get(
        "vindr_dicom_root",
        None,
    )

    if has_vindr and vindr_dicom_root:
        vindr_train = DicomFourViewDataset(
            vindr_train_manifest,
    ...
    if vindr_train is not None:
        ...
        # TN-only training (DICOM mode)
        combined_train = tn_train
        domains = None
    ...
    if domains is not None:
        # Multi-domain with weighted sampler
        tn_ratio = float(
            data_config[
                "tn_domain_ratio"
            ]
        ...
        # TN-only
        theoretical_mass = {"TN": 1.0}
        
        if training_config.get("oversample_classes", False):
            print("[ENGINE] Using class oversampling for TN-only training.")
            labels = tn_train.dataframe["label"].map({"A": 0, "B": 1, "C": 2, "D": 3}).tolist()
            from collections import Counter
            ...
            num_samples = int(training_config.get("sampler_num_samples", len(combined_train)))
...
def build_model(
    ...
    options = ModelOptions(
        ...
        use_cd_head=bool(
            model_config.get("use_cd_head", False)
        ),
        imagenet_init=bool(
            model_config.get(
                "imagenet_init", False
        ...
        control_hidden_dim=int(
            model_config.get(
                "control_hidden_dim", 608
            )
        ),
        bilateral_bottleneck_dim=int(
            model_config.get(
                "bilateral_bottleneck_dim", 256
            )
        ),
        use_cbam=bool(
            model_config.get("use_cbam", False)
        ),
        use_fpn=bool(
            model_config.get("use_fpn", False)
        ),
        freeze_early_layers=bool(
            model_config.get("freeze_early_layers", False)
        ),
        backbone=str(
            model_config.get("backbone", "densenet121")
        ),
        region_enabled=bool(
            model_config.get("region_enabled", False)
        ),
        region_dim=int(
            model_config.get("region_dim", 256)
        ),
        region_max_tokens=int(
            model_config.get("region_max_tokens", 256)
        ),
        region_num_heads=int(
            model_config.get("region_num_heads", 8)
        ),
        region_contralateral=bool(
            model_config.get("region_contralateral", True)
        ),
        region_ipsilateral=bool(
            model_config.get("region_ipsilateral", False)
        ),
        region_alpha_init=float(
            model_config.get("region_alpha_init", 0.0)
        ),
        bc_aux_enabled=bool(
            model_config.get("bc_aux_enabled", False)
        ),
    )

    if options.backbone == "catbeo":
        model = CatBeoDensityModel(options)
    else:
    ...
    if checkpoint_path_raw:
        ...
        # Remove backbone.classifier to avoid size mismatch
        # since it is never used for feature extraction
        state_dict.pop("backbone.classifier.weight", None)
        state_dict.pop("backbone.classifier.bias", None)
...
def build_criterion(
    ...
    return MultiTaskCriterion(
        ...
        options=MultiTaskLossOptions(
            ...
            lambda_neighbor_penalty=float(
                loss_config.get(
                    "lambda_neighbor_penalty",
                    0.0,
                )
            ),
            ...
            cd_class_counts=(
                [int(v) for v in loss_config["cd_class_counts"]]
                if "cd_class_counts" in loss_config
                else None
            ),
            lambda_bc=float(
                loss_config.get("lambda_bc", 0.0)
            ),
            bc_class_counts=(
                [int(v) for v in loss_config["bc_class_counts"]]
                if "bc_class_counts" in loss_config
                else None
            ),
        ),
    )
